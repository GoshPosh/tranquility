...
/**
 * tranquility-kinesis  main.
 */
@Command(name = "tranquility-kinesis", description = "Kinesis consumer which pushes events to Druid through Tranquility")
public class KinesisMain
{
  ...
  public void run() throws InterruptedException
  {
    ...
    for (String propertyName : config.globalConfig().properties().stringPropertyNames()) {
      if (propertyName.startsWith("kinesis.")) {
        kinesisProperties.setProperty(
            propertyName.replaceFirst("kinesis\\.", ""),
            config.globalConfig().properties().getProperty(propertyName)
        );
      }
    }

    // set the critical Kinesis configs again from TranquilityKinesisConfig so it picks up the defaults
    kinesisProperties.setProperty("appName", globalConfig.getKinesisAppName());
//    kinesisProperties.setProperty("zookeeper.connect", globalConfig.getKinesisZookeeperConnect());
    if (kinesisProperties.setProperty(
    ...
    final TranquilityEventWriter writer = new TranquilityEventWriter(dataSourceConfigs.values().iterator().next());
    final KinesisConsumer kinesisConsumer = new KinesisConsumer(
        globalConfig,
        kinesisProperties,
        dataSourceConfigs,
            writer
